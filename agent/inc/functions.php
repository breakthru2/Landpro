...
function register($post){
	...
	if ($err_flag === false) {
		$sql = "INSERT INTO agents (firstname, lastname, email, password, tel, address, union_id_fk) VALUES ('$firstname', '$lastname', '$email', '$password', '$tel', '$address', 1)";
		$query = mysqli_query($link, $sql);
...
function lockscreen($post){
	...
	if ($err_flag === false) {
		$sql = "SELECT password FROM agents WHERE password = '$password'";
		$query = mysqli_query($link, $sql);
